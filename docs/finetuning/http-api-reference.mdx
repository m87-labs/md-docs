...
| spatial_refs | array | no | Normalized point or box references for grounding |
...
For `point` and `detect`, the output token budget is derived from `max_objects`. Public requests do not accept `max_tokens` for those skills.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| mode | string | no | `"rl"` or `"sft"` |
| request | object | yes | Rollouts request for RL, or skill request for SFT |
| rollouts | array | RL only | Rollout objects from `/rollouts` (pass unchanged) |
| rewards | array | RL only | Reward for each rollout (same length and order) |
| targets | array | SFT only | Target objects for supervised training |
