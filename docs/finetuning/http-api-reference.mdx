...
| Skill | Type | Required | Description |
|-------|------|----------|-------------|
| `query` | object | `answer` | `reasoning` is required when `request.reasoning=true`; omit it when `request.reasoning=false` |
| `point` | object | `points` or `boxes` | Include exactly one of `points` or `boxes` |
| `detect` | object | `boxes` | One or more target boxes |
...
[
  {
    "answer": "United States",
    "reasoning": "The road markings and signs match the US."
  }
]
```
...
Additional training metrics may also be returned.
