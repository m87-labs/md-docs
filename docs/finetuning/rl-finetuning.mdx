...
| **Adapter** | The finetuned weights that layer on top of the base model (using LoRA). Identified by a unique `adapter_id`. |
| **Checkpoint** | A saved adapter state at a specific training step. Created automatically after each train step. |
| **Release** | A checkpoint promoted for production use. Releases are persistent and optimized for inference. |
| **Train step** | One update to the model based on a batch of scored rollouts |

### Checkpoint lifecycle

Checkpoints are created automatically when you apply a train step. By default, they expire after 24 hours to save storage. You can:

- **Pin** a checkpoint to keep it indefinitely
- **Release** a checkpoint to make it permanently available for inference
- **Delete** a checkpoint early by setting its expiration to the past

The latest checkpoint per adapter is never automatically deleted, ensuring you can always resume training.
...
Once you're happy with your model's performance, release a checkpoint to make it available for production:

```
POST /v1/tuning/adapters/:adapterId/checkpoints/:step/release
```

This creates an optimized, persistent version of your weights.

To run inference with your finetuned adapter, use the `/rollouts` endpoint with `num_rollouts: 1` and `temperature: 0` for deterministic output:

```json
// POST /v1/tuning/rollouts
{
  "adapter_id": "01HXYZ...",
  "num_rollouts": 1,
  "request": {
...
The response contains the model's prediction in the `rollouts[0].output` field.

Support for using finetuned adapters with the standard inference endpoints (`/query`, `/point`, `/detect`) is coming soon.
