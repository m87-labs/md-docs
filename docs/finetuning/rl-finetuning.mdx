...
The latest checkpoint per finetune is always available for resuming training.
...
To run inference with your finetune, use the `/rollouts` endpoint with `num_rollouts: 1` and `temperature: 0` for deterministic output:
...
Support for using finetunes with the standard inference endpoints (`/query`, `/point`, `/detect`) is coming soon.
