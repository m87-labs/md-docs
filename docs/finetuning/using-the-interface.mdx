...
- **`rank`** — the LoRA rank (higher = more capacity, but slower and more memory).
...
The response returns training metrics (loss, average reward, etc.) that you can use to monitor learning and detect instability.
...
- See the **HTTP API reference** for full schemas of all skills (`query`, `point`, `detect`).
