...
- **`adapter`** — a unique identifier for this adapter (alphanumeric, hyphens, and underscores only).
- **`rank`** — the LoRA rank: 8, 16, 24, or 32 (must be a multiple of 8; higher = more capacity, but slower).

The response includes:

- **`adapter`** — the adapter identifier you provided
- **`rank`** — the LoRA rank
- **`created`** — `true` if newly created, `false` if an adapter with this name already existed
...
- **`adapter`** — the adapter to use (from the create adapter call).
